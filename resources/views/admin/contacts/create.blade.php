    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.contacts.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('responsible_user') ? 'is-invalid' : '' }}" name="responsible_user_id" id="responsible_user_id">
                    @foreach($responsible_users as $id => $entry)
                        <option value="{{ $id }}" {{ old('responsible_user_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('meeting_date') ? 'is-invalid' : '' }}" type="text" name="meeting_date" id="meeting_date" value="{{ old('meeting_date') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('budget') ? 'is-invalid' : '' }}" name="budget" id="budget" required>
                    ...
                    @foreach(App\Models\Contact::BUDGET_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('budget', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('number_of_bedrooms') ? 'is-invalid' : '' }}" name="number_of_bedrooms" id="number_of_bedrooms" required>
                    ...
                    @foreach(App\Models\Contact::NUMBER_OF_BEDROOMS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('number_of_bedrooms', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
